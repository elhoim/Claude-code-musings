...
import { SPANISH_A1_STORIES } from '../src/db/seeds/spanish-a1-stories';
...
describe('SPANISH_A1_STORIES', () => {
  it('has exactly 3 stories', () => {
    expect(SPANISH_A1_STORIES).toHaveLength(3);
  });

  describe('each story has all required fields', () => {
    SPANISH_A1_STORIES.forEach((story, index) => {
  ...
  describe('segments with choices have valid nextSegmentOrder references', () => {
    SPANISH_A1_STORIES.forEach((story, storyIndex) => {
  ...
  it('at least one story has a segment with promptForUser', () => {
    const hasPrompt = SPANISH_A1_STORIES.some((story) =>
