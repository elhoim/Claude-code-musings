...
import { FRENCH_A1_STORIES } from '../src/db/seeds/french-a1-stories';
...
describe('FRENCH_A1_STORIES', () => {
  it('has exactly 3 stories', () => {
    expect(FRENCH_A1_STORIES).toHaveLength(3);
  });

  describe('each story has all required fields', () => {
    FRENCH_A1_STORIES.forEach((story, index) => {
  ...
  describe('segments with choices have valid nextSegmentOrder references', () => {
    FRENCH_A1_STORIES.forEach((story, storyIndex) => {
  ...
  it('at least one story has a segment with promptForUser', () => {
    const hasPrompt = FRENCH_A1_STORIES.some((story) =>
