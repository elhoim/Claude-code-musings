...
import { FLEMISH_A1_STORIES } from '../src/db/seeds/flemish-a1-stories';
...
describe('FLEMISH_A1_STORIES', () => {
  it('has exactly 3 stories', () => {
    expect(FLEMISH_A1_STORIES).toHaveLength(3);
  });

  describe('each story has all required fields', () => {
    FLEMISH_A1_STORIES.forEach((story, index) => {
  ...
  describe('segments with choices have valid nextSegmentOrder references', () => {
    FLEMISH_A1_STORIES.forEach((story, storyIndex) => {
  ...
  it('at least one story has a segment with promptForUser', () => {
    const hasPrompt = FLEMISH_A1_STORIES.some((story) =>
