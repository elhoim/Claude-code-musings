...
} from '../src/db/seeds/french-a1-content';
...
describe('FRENCH_A1_UNITS', () => {
  it('has exactly 4 units', () => {
    expect(FRENCH_A1_UNITS).toHaveLength(4);
  });

  describe('each unit has 5 lessons', () => {
    FRENCH_A1_UNITS.forEach((unit, unitIndex) => {
...
describe('FRENCH_A1_PLACEMENT_QUESTIONS', () => {
  it('has questions for A1 level', () => {
    const a1Questions = FRENCH_A1_PLACEMENT_QUESTIONS.filter(
      (q) => q.cefrLevel === 'A1',
    );
  ...
  it('has questions for A2 level', () => {
    const a2Questions = FRENCH_A1_PLACEMENT_QUESTIONS.filter(
      (q) => q.cefrLevel === 'A2',
    );
  ...
  it('has questions for B1 level', () => {
    const b1Questions = FRENCH_A1_PLACEMENT_QUESTIONS.filter(
      (q) => q.cefrLevel === 'B1',
    );
  ...
  describe('each placement question has a distractors array', () => {
    FRENCH_A1_PLACEMENT_QUESTIONS.forEach((question, index) => {
