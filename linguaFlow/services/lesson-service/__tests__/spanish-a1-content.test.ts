...
} from '../src/db/seeds/spanish-a1-content';
...
describe('SPANISH_A1_UNITS', () => {
  it('has exactly 4 units', () => {
    expect(SPANISH_A1_UNITS).toHaveLength(4);
  });

  describe('each unit has 5 lessons', () => {
    SPANISH_A1_UNITS.forEach((unit, unitIndex) => {
...
describe('SPANISH_A1_PLACEMENT_QUESTIONS', () => {
  it('has questions for A1 level', () => {
    const a1Questions = SPANISH_A1_PLACEMENT_QUESTIONS.filter(
      (q) => q.cefrLevel === 'A1',
    );
  ...
  it('has questions for A2 level', () => {
    const a2Questions = SPANISH_A1_PLACEMENT_QUESTIONS.filter(
      (q) => q.cefrLevel === 'A2',
    );
  ...
  it('has questions for B1 level', () => {
    const b1Questions = SPANISH_A1_PLACEMENT_QUESTIONS.filter(
      (q) => q.cefrLevel === 'B1',
    );
  ...
  describe('each placement question has a distractors array', () => {
    SPANISH_A1_PLACEMENT_QUESTIONS.forEach((question, index) => {
