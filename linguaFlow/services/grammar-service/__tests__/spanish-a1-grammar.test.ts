...
import {
  SPANISH_A1_GRAMMAR_NODES,
  buildGrammarEdges,
} from '../src/db/seeds/spanish-a1-grammar';
...
describe('SPANISH_A1_GRAMMAR_NODES', () => {
  it('has exactly 16 nodes', () => {
    expect(SPANISH_A1_GRAMMAR_NODES).toHaveLength(16);
  });

  describe('each node has all required fields', () => {
    SPANISH_A1_GRAMMAR_NODES.forEach((node, index) => {
  ...
  it('all slugs are unique', () => {
    const slugs = SPANISH_A1_GRAMMAR_NODES.map((n) => n.slug);
    const uniqueSlugs = new Set(slugs);
    expect(uniqueSlugs.size).toBe(slugs.length);
  });

  describe('prerequisites reference valid slugs (no dangling references)', () => {
    const allSlugs = new Set(SPANISH_A1_GRAMMAR_NODES.map((n) => n.slug));

    SPANISH_A1_GRAMMAR_NODES.forEach((node) => {
...
describe('buildGrammarEdges', () => {
  const edges = buildGrammarEdges(SPANISH_A1_GRAMMAR_NODES);

  it('produces the correct number of edges', () => {
    const expectedEdgeCount = SPANISH_A1_GRAMMAR_NODES.reduce(
