...
import {
  ENGLISH_A1_GRAMMAR_NODES,
  buildGrammarEdges,
} from '../src/db/seeds/english-a1-grammar';
...
describe('ENGLISH_A1_GRAMMAR_NODES', () => {
  it('has exactly 16 nodes', () => {
    expect(ENGLISH_A1_GRAMMAR_NODES).toHaveLength(16);
  });

  describe('each node has all required fields', () => {
    ENGLISH_A1_GRAMMAR_NODES.forEach((node, index) => {
  ...
  it('all slugs are unique', () => {
    const slugs = ENGLISH_A1_GRAMMAR_NODES.map((n) => n.slug);
    const uniqueSlugs = new Set(slugs);
    expect(uniqueSlugs.size).toBe(slugs.length);
  });

  describe('prerequisites reference valid slugs (no dangling references)', () => {
    const allSlugs = new Set(ENGLISH_A1_GRAMMAR_NODES.map((n) => n.slug));

    ENGLISH_A1_GRAMMAR_NODES.forEach((node) => {
...
describe('buildGrammarEdges', () => {
  const edges = buildGrammarEdges(ENGLISH_A1_GRAMMAR_NODES);

  it('produces the correct number of edges', () => {
    const expectedEdgeCount = ENGLISH_A1_GRAMMAR_NODES.reduce(
